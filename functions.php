...
function getPrice($price){
    ...
    if (!is_numeric($price)) {
        return ''; // Handle invalid price (optional, you can also throw an error or return a default value)
    }
...
function getServices($connect, $lang){
    
    $sentence = $connect->prepare("SELECT services.*,tr_services.* FROM services,tr_services WHERE services.service_id = tr_services.tr_service AND tr_services.tr_lang = '".$lang."'"); 
    $sentence->execute();
    return $sentence->fetchAll();
}

function getLatestServices($connect, $lang){
    
    $sentence = $connect->prepare("SELECT services.*,tr_services.* FROM services,tr_services WHERE services.service_id = tr_services.tr_service AND tr_services.tr_lang = '".$lang."' LIMIT 6"); 
    $sentence->execute();
    return $sentence->fetchAll();
}
...
/*Certificates*/
function getCertificates($connect){
    $sentence = $connect->query("SELECT * FROM certificates WHERE certificate_status = 1");
    $sentence->execute();
    return $sentence->fetchAll();
}

function getLatestCertificates($connect){
    $sentence = $connect->query("SELECT * FROM certificates WHERE certificate_status = 1 LIMIT 4");
    $sentence->execute();
    return $sentence->fetchAll();
}
