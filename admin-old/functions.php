...
function get_all_preferred_choice($connect, $items_per_page = NULL)
{
    ...
    $sql = "SELECT preferred_choice.*,tr_preferred_choice.* FROM preferred_choice,tr_preferred_choice,settings WHERE preferred_choice.pc_id = tr_preferred_choice.tr_pc AND tr_preferred_choice.tr_lang = settings.st_language GROUP BY tr_preferred_choice.tr_pc"; 
...
function get_all_staffs($connect, $items_per_page = NULL)
{
    ...
    $sql = "SELECT staffs.*,tr_staffs.* FROM staffs,tr_staffs,settings WHERE staffs.staff_id = tr_staffs.tr_staff AND tr_staffs.tr_lang = settings.st_language GROUP BY tr_staffs.tr_staff"; 
...
function get_all_users($connect, $items_per_page)
{
    
    $limit = (currentPage() > 1) ? currentPage() * $items_per_page - $items_per_page : 0;
    
    $sentence = $connect->prepare("SELECT users.*,roles.role_name AS role_name FROM users,roles WHERE users.user_role = roles.role_id ORDER BY user_id ASC LIMIT $limit, $items_per_page"); 
    $sentence->execute();
    return $sentence->fetchAll();
}
...
function get_all_subscribers($connect, $items_per_page)
{
    
    $limit = (currentPage() > 1) ? currentPage() * $items_per_page - $items_per_page : 0;
    
    $sentence = $connect->prepare("SELECT * FROM subscribers ORDER BY subscriber_id ASC LIMIT $limit, $items_per_page"); 
    $sentence->execute();
    return $sentence->fetchAll();
}
// SERVICES ---------------------------------------
...
function get_services_languages($connect)
{
    
    $sentence = $connect->prepare("SELECT * FROM languages WHERE language_status = 1 AND language_code IN (SELECT tr_lang FROM tr_services) ORDER BY language_id ASC"); 
    $sentence->execute();
    return $sentence->fetchAll();
}

function totalServices($connect, $items_per_page){

    $total_items = $connect->prepare("SELECT COUNT(*) AS total FROM services");
    $total_items->execute();
    $total_items = $total_items->fetch()['total'];
...
function get_active_services($connect)
{
    
    $sentence = $connect->prepare("SELECT * FROM services WHERE service_status = 1 ORDER BY service_id ASC"); 
    $sentence->execute();
    return $sentence->fetchAll();
}

function get_all_services($connect, $items_per_page = NULL)
{
    
    $limit = (currentPage() > 1) ? currentPage() * $items_per_page - $items_per_page : 0;
    
    $sql = "SELECT services.*,tr_services.* FROM services,tr_services,settings WHERE services.service_id = tr_services.tr_service AND tr_services.tr_lang = settings.st_language GROUP BY tr_services.tr_service"; 
...
function id_service($id_service){
    return (int)cleardata($id_service);
}

function get_languages_by_service($connect, $id_service)
{
    $sentence = $connect->prepare("SELECT * FROM languages WHERE language_status = 1 AND language_code IN (SELECT tr_lang FROM tr_services WHERE tr_service = $id_service) ORDER BY language_id ASC"); 
    $sentence->execute();
    return $sentence->fetchAll();
}

function get_service_per_id_by_language($connect, $id_service, $lang){
    $sentence = $connect->query("SELECT services.*,tr_services.* FROM services,tr_services WHERE services.service_id = $id_service AND tr_services.tr_service = $id_service AND tr_services.tr_lang = '".$lang."' LIMIT 1");
    $sentence = $sentence->fetchAll();
    return ($sentence) ? $sentence : false;
}

function check_service($connect, $id_service, $lang){
    $sentence = $connect->query("SELECT * FROM tr_services WHERE tr_service = $id_service AND tr_lang = '".$lang."' LIMIT 1");
    $sentence = $sentence->fetchAll();
    return ($sentence) ? $sentence : false;
}


function get_languages_not_services($connect, $id_service)
{
    
    $sentence = $connect->prepare("SELECT * FROM languages WHERE language_status = 1 AND language_code NOT IN (SELECT tr_lang FROM tr_services WHERE tr_service = $id_service) ORDER BY language_id ASC"); 
    $sentence->execute();
    return $sentence->fetchAll();
}

function get_service_slug($connect, $slug){

$sentence = $connect->prepare("SELECT COUNT(*) AS total FROM tr_services WHERE tr_slug LIKE '$slug%'");
$sentence->execute();
$row = $sentence->fetch(PDO::FETCH_ASSOC);
return $row['total'];
}
...
function totalCertificates($connect, $items_per_page){

    $total_items = $connect->prepare("SELECT COUNT(*) AS total FROM certificates");
    $total_items->execute();
    $total_items = $total_items->fetch()['total'];
...
function get_all_certificates($connect, $items_per_page = NULL)
{
...
function id_certificate($id_certificate){
    return (int)cleardata($id_certificate);
}

function get_certificate_per_id($connect, $id_certificate){
    $sentence = $connect->query("SELECT * FROM certificates WHERE certificate_id = $id_certificate LIMIT 1");
    $sentence = $sentence->fetchAll();
    return ($sentence) ? $sentence : false;
}
...
function get_all_testimonials($connect, $items_per_page){
    
    $limit = (currentPage() > 1) ? currentPage() * $items_per_page - $items_per_page : 0;
    
    $sentence = $connect->prepare("SELECT testimonials.*, languages.language_name FROM testimonials, languages WHERE testimonials.testimonial_lang = languages.language_code LIMIT $limit, $items_per_page"); 
    $sentence->execute();
    return $sentence->fetchAll();
}
...
function get_all_sliders($connect, $items_per_page)
{
    
    $limit = (currentPage() > 1) ? currentPage() * $items_per_page - $items_per_page : 0;
    
    $sentence = $connect->prepare("SELECT sliders.*, properties.pt_reference AS property_reference FROM sliders, properties WHERE sliders.slider_property = properties.pt_id LIMIT $limit, $items_per_page"); 
    $sentence->execute();
    return $sentence->fetchAll();
}
